...
- Accounts needed:
  ...
  - [Inngest](https://cwa.run/inngest) - Background jobs (optional)
  - [Zhipu AI](https://open.bigmodel.cn) - AI API (GLM-4.7)
...
4. Configure your `.env.local` with the required keys:
   ...
   NEXT_SUPABASE_PUBLISHABLE_KEY=your_supabase_anon_key

   # Convex
   NEXT_PUBLIC_CONVEX_URL=your_convex_deploy_url
...
7. In another terminal, start the Inngest dev server (optional):
...
**[Supabase](https://supabase.com)** - Add authentication to your app with open source Firebase alternative.
